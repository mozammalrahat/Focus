...
router.post("/like/:postId", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(200).send("Post voted");
  } catch (error) {
    console.error(error);
    return res.status(500).send(`Server error`);
  }
...
router.put("/unlike/:postId", authMiddleware, async (req, res) => {
  try {
    ...
    if (isLiked) {
      return res.status(401).send("Post not voted before");
    }
...
router.get("/like/:postId", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(200).json(post.votes);
  } catch (error) {
    console.error(error);
    return res.status(500).send(`Server error`);
  }
...
router.post("/comment/:postId", authMiddleware, async (req, res) => {
  try {
    ...
    if (text.length < 1)
      return res.status(401).send("Answer should be atleast 1 character");
